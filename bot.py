...
from discord import slash_command, Interaction, SlashOption
...
handleConsole.setFormatter(logging.Formatter('%(asctime)s |:| %(module)s | %(message)s', "%H:%M:%S"))
log.addHandler(handleConsole)

handleFile = logging.FileHandler(filename=f'logs{os.sep}discordGeneral.log', encoding='utf-8', mode='a')
handleFile.setFormatter(logging.Formatter('%(asctime)s:%(created)f |:| %(levelname)s:%(module)s; %(funcName)s | %(message)s',))
log.addHandler(handleFile)

messHandler = logging.FileHandler(filename=f'logs{os.sep}discordMessages.log', encoding='utf-8', mode='a')
...
def main():	
	...
	activity = nextcord.Activity(
		type=nextcord.ActivityType.listening, name=f"{config.BOT_PREFIX}help | v{verMajor}.{verMinor}")
...
Nextcord v{nextcord.__version__} **Ready**\nUse /changelog to see changes"""
		...
		oldVerMajor = config['General']['verMajor']
		oldVerMinor = config['General']['VerMinor']
		oldVerPoint = config['General']['verPoint']
		if (oldVerMajor != verMajor
		or oldVerMinor != verMinor
		or oldVerPoint != verPoint):
			config['General']['verMajor'] = oldVerMajor
			config['General']['VerMinor'] = oldVerMinor
			config['General']['verPoint'] = oldVerPoint
			config['General']['verName'] = verName
			writeJSON(data=config, filename="config")
		if configGen['readyMessage'] is False: return
		configTPF = configuration['TPFGuild']
		configNIX = configuration['NIXGuild']
		sendReady = {
			configTPF['Channels']['Botstuff']:configTPF['readyMessage'],
			configNIX['Channels']['Botstuff']:configNIX['readyMessage']
		}
