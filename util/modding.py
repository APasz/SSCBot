...
globalPreviewChan = configuration["TPFGuild"]["Channels"]["NewModPreview"]


async def modPreview(
    dets, chan, globalPreviewChan, platform, img=None, globalPreview: bool = False
):
    ...
    if platform == "steam":
        ...
        if tags is not None:
            tags = ", ".join(tags)
        e = nextcord.Embed(
            title=f"Workshop Mod Published;\n{modName}", colour=getCol("steam"), url=url
        )
        ...
        e = nextcord.Embed(
            title=f"TPF|NET Mod Published;\n{name}", colour=getCol("tpfnet"), url=url
        )
    ...
    if globalPreview is True:
        ...
        await globalPreviewChan.send(embed=e)


async def modRelease(
    ctx, chan, globalPreviewChan=globalPreviewChan, globalPreview: bool = False
):
